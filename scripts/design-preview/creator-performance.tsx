...
import { historyDays, historyPoints } from '../../src/lib/data/creator-performance-history-model';
import './preview.css';
...
const adapterDays = historyDays('2025-09-01', '2026-08-31');
const adapterPoints = historyPoints(adapterDays, adapterDays.map((stat_date, i) => ({ stat_date, gmv: 200 + i * 3, posts: i % 3 === 0 ? 1 : 0 })));
function Preview() {
  const [fixture, setFixture] = useState('year');
  const [dark, setDark] = useState(false);
  const points = fixture === 'adapter' ? adapterPoints : fixture === 'empty' ? [] : fixture === 'single' ? baseline.slice(0, 1) : baseline.map((point, i) => ({ ...point,
