...
function load(file,deps={},extra={}){const exports={};runInNewContext(ts.transpileModule(readFileSync(file,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports,process,Date,AbortSignal,setTimeout,require:n=>{assert.ok(n in deps,n);return deps[n];},...extra});return exports;}
...
let acquired=0,calls=0,discordStatus=200,limitedOnce=false;
...
discordStatus=200;limitedOnce=true;result=await route.GET(req('jiyu'));assert.equal(result.body.rows[0].state,'aligned');assert.equal(result.body.rateLimited,false);
