-- Precompute handle/day membership to preserve ordinary joins over sales facts.
...
scoped_daily_video_product_stats AS NOT MATERIALIZED (SELECT * FROM public.daily_video_product_stats WHERE tenant_id = p_tenant_id), roster as (
  ...
  roster_handles as materialized (
    -- Resolve membership once per handle/day, not once per sales fact.
    select distinct on (handle, p_start + days.n)
           handle, id, p_start + days.n as report_date
    from handle_src cross join generate_series(0, p_end - p_start) as days(n)
    where (archived_on is null or p_start + days.n < archived_on)
      and (reporting_start_date is null or p_start + days.n >= reporting_start_date)
    order by handle, p_start + days.n, (archived_on is null) desc, (creator_id is not null) desc, id
  ),
  facts as (
    select dv.video_id,
           dv.post_date,
    ...
    left join roster_handles rh
      ...
      and rh.report_date = dv.report_date
