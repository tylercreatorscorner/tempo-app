-- Auth identity is resolved by the server through the Auth Admin API before this RPC.
-- service_role deliberately has no direct access to auth.users in hosted Supabase.
-- Keep SECURITY INVOKER and the service-only grant; do not widen auth table privileges.
-- Foreign keys enforce account existence and profile row locks preserve tenant ownership.
...
DECLARE actor_tenant uuid; target_tenant uuid; target_role text;
BEGIN
  ...
  IF p_email IS NULL OR length(trim(p_email)) = 0 THEN
    RAISE EXCEPTION 'Invalid account email' USING ERRCODE='42501';
  END IF;
  ...
  ELSE
    ...
    INSERT INTO public.user_profiles(user_id,email,role,role_id,tenant_id,status,can_view_finance)
      VALUES(p_user_id,lower(trim(p_email)),p_role,NULL,actor_tenant,'active',
