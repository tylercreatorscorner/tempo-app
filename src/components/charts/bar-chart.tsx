  ...
  return (
    <div className={className}>
      <div className="space-y-2.5">
        {rows.map((r, ri) => (
          <div key={ri} className="grid grid-cols-[minmax(70px,120px)_1fr_auto] items-center gap-3" onMouseEnter={() => setHover(ri)} onMouseLeave={() => setHover(null)}>
            ...
            {hover === ri && stacked && r.segments && (
              <div className="pointer-events-none absolute right-4 z-10 mt-8 rounded-lg border border-border bg-popover px-2.5 py-1.5 text-[11px] shadow-lg">
                ...
                {r.segments.map((s, si) => (
                  <div key={si} className="flex items-center gap-1.5">
                    ...
                    <span className="ml-auto pl-3 font-mono tabular-nums text-foreground">{fmt(s.value)}</span>
                  </div>
                ))}
              </div>
            )}
