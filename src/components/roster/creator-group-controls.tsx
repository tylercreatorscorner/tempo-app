...
export function CreatorGroupControls({brand,rows,selected,mode,onFilter,onSaved,loading}:{brand:string;rows:Row[];selected:string[];mode:'any'|'all';onFilter:(tags:string[],mode:'any'|'all')=>void;onSaved:()=>void;loading:boolean}) {
  ...
  const [canReviewDiscord,setCanReviewDiscord]=useState(false);
  ...
  useEffect(()=>{
    let ignore=false;
    fetch(`/api/roster/tags?brand=${encodeURIComponent(brand)}`).then(async r=>{const j=await r.json();if(!r.ok)throw new Error(j.error);if(!ignore){setTags(j.tags);setCanWrite(j.canWrite);setCanReviewDiscord(!!j.canReviewDiscord);}}).catch(()=>{if(!ignore){setCanWrite(false);setError('Could not load creator tags.');}});
    return ()=>{ignore=true;};
  },[brand,version]);
  ...
  return <section className="flex items-center gap-1" aria-label="Creator groups">
    ...
    {canReviewDiscord && brand==='jiyu' && <Link className="rounded-lg px-3 py-2 text-xs font-medium hover:bg-secondary" href="/roster/discord-sync?brand=jiyu">Discord roles</Link>}
