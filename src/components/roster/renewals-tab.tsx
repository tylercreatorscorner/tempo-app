  ...
  if (loading && !data) {
    return (
      <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-12 text-center">
        ...
        <p className="text-sm text-gray-500 mt-3">Computing renewals…</p>
      </div>
    );
  }
...
function HeadlineCell({
  ...
  return (
    <div>
      ...
      <div className="text-2xl font-extrabold text-[#1A1B3A]">{value}</div>
    ...
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className={cn(
        ...
        >
          ...
          <div className="text-left">
            <div className="text-sm font-bold text-[#1A1B3A]">
              {section.label} · <span className={cn('font-extrabold', ACCENT_TEXT[section.accent])}>{creators.length}</span>
            </div>
            <div className="text-[11px] text-gray-500">{section.sublabel}</div>
          </div>
        </button>
        <button
          onClick={handleCopy}
          ...
          className={cn(
            ...
            copied ? 'bg-emerald-500 text-white' : 'bg-white border border-gray-200 hover:bg-gray-50 text-gray-700',
      ...
      {!isCollapsed && (
        <div className="divide-y divide-gray-50">
          {creators.length === 0 ? (
            <div className="text-sm text-gray-400 py-8 text-center">No creators in this bucket.</div>
          ) : (
...
function RenewalRow({ creator: c }: { creator: RenewalCreator }) {
  ...
  return (
    <a
      href={profileHref}
      className="grid grid-cols-12 gap-3 items-center px-5 py-3 hover:bg-gray-50/60 transition-colors"
    >
      {/* Creator identity */}
      <div className="col-span-12 sm:col-span-3 flex items-center gap-2 min-w-0">
        ...
        <div className="min-w-0">
          <div className="text-sm font-semibold text-[#1A1B3A] truncate flex items-center gap-1">
            @{handleClean}
      ...
      <div className="col-span-3 sm:col-span-2 text-right sm:text-left">
        ...
        <div className="text-[10px] text-gray-400 mt-0.5">ROI</div>
      </div>

      {/* GMV */}
      <div className="col-span-3 sm:col-span-2 text-right sm:text-left">
        <div className="text-sm font-bold text-[#1A1B3A] tabular-nums">{formatCurrency(c.gmv)}</div>
        <div className="text-[10px] text-gray-400 mt-0.5">{formatCurrency(c.retainer)}/mo retainer</div>
      </div>
      ...
      <div className="col-span-3 sm:col-span-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-[#1A1B3A] tabular-nums whitespace-nowrap">
      ...
      <div className="col-span-3 sm:col-span-2 text-right">
        {daysLeft !== null ? (
          <>
            <div className="text-sm font-semibold text-[#1A1B3A] tabular-nums">
              {daysLeft === 0 ? 'Today' : `${daysLeft}d`}
            </div>
            <div className="text-[10px] text-gray-400 mt-0.5">until renewal</div>
          </>
        ) : (
          <>
            <div className="text-sm font-semibold text-gray-500 tabular-nums">Rolling</div>
            <div className="text-[10px] text-gray-400 mt-0.5">{c.contractLengthDays}d window</div>
          </>
        )}
      </div>
    </a>
