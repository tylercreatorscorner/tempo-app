    ...
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold text-[#1A1B3A]">Recent uploads</div>
        </div>
        <div className="text-[11px] text-gray-400">{items.length} of last 20</div>
      </div>
        ...
        <ul className="divide-y divide-gray-50">
          {items.map(it => (
            <li key={it.id} className="px-4 py-3 flex items-center gap-4 text-sm">
              <span className="text-[11px] text-gray-400 w-16 shrink-0">{relativeTime(it.createdAt)}</span>
              <span className="font-medium text-[#1A1B3A] w-32 shrink-0 truncate" title={it.brandLabel}>
                {it.brandLabel || '—'}
              </span>
              ...
              <span className="text-[11px] text-gray-500 ml-auto whitespace-nowrap">
                <strong className="text-gray-700">{it.rowCount.toLocaleString()}</strong> rows
              </span>
              <span className="text-[11px] text-gray-400 truncate max-w-[120px]" title={it.uploadedBy}>
                by {it.uploadedBy}
              </span>
            </li>
          ))}
