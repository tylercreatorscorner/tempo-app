...
export function BrandPortrait({ name, source, size = 30 }: { name: string; source?: string | null; size?: number }) {
  ...
  if(source && failed !== source) return <img src={source} alt="" referrerPolicy="no-referrer" onError={()=>setFailed(source)} style={{...style,objectFit:'contain',background:'var(--secondary)',padding:2}} />;
