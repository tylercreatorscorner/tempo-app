...
export function GmvContributors({ rows, available, brand, coverage }: { rows: Contributor[]; available: boolean; brand: string | null; coverage: string }) {
  return <section className="rounded-2xl border border-border bg-card p-4" aria-label="Managed GMV contributors">
    ...
    {!available ? <p className="text-xs text-muted-foreground">Creator comparisons will appear when both periods have recorded daily coverage.</p> : !rows.length ? <p className="text-xs text-muted-foreground">No managed creator changes of $1 or more in this period.</p> : <div className="grid gap-3 md:grid-cols-3">{rows.slice(0,3).map(row => <Link key={row.key} href={`/creators/${encodeURIComponent(row.id ?? row.handle)}${brand ? `?brand=${encodeURIComponent(brand)}` : ''}`} className="flex items-center gap-3 rounded-xl border border-border p-3 hover:bg-muted focus-visible:outline-primary">
