...
function NotableCardSkeleton() {
  return (
    <div className="rounded-2xl bg-card border border-border shadow-sm p-4">
      ...
      <div className="h-3.5 w-32 rounded bg-secondary animate-pulse mb-1" />
      <div className="h-3 w-24 rounded bg-muted animate-pulse mb-3" />
      <div className="flex items-baseline justify-between">
        <div>
          <div className="h-2.5 w-10 rounded bg-muted animate-pulse mb-1" />
...
export default function AnalyticsLoading() {
  return (
    <div className="space-y-6">
      ...
      <div className="h-9 w-full rounded-full bg-muted animate-pulse" />

      {/* KPI strip — 7 placeholder cards (hero spans 2 cols + 6 standard) */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
        <div className="rounded-2xl bg-gradient-to-br from-[var(--foreground)]/95 via-[#2D1B69]/95 to-[var(--foreground)]/95 p-5 col-span-2 sm:col-span-2 lg:col-span-2">
          <div className="h-3 w-20 rounded bg-card/20 animate-pulse mb-2" />
          <div className="h-7 w-24 rounded bg-card/20 animate-pulse" />
        </div>
      ...
      <div className="rounded-2xl bg-card border border-border shadow-sm p-5">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-xl bg-muted px-3 py-2.5">
              <div className="h-2.5 w-12 rounded bg-secondary animate-pulse mb-1" />
              <div className="h-4 w-10 rounded bg-secondary animate-pulse mb-1" />
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <li key={i} className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <span className="h-2.5 w-2.5 rounded-full bg-secondary animate-pulse flex-shrink-0" />
